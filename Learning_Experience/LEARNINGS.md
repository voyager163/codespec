...
| [L004](./L004-search-all-copies-before-scoping-a-change.md) | major | Scoped `remove-demo-theater` to the dev copy only, missing the git-tracked `templates/starter/` duplicate that ships to generated apps | Grep the WHOLE tree for every symbol being removed before scoping/closing a change; treat each tracked vendored/starter copy as in-scope; never cite "N sites" from in-context files alone. |
